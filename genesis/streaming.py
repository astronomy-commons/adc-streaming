...
_MESSAGE_PARSERS = {"avro": parse_avro, "json": parse_json, "blob": parse_blob}
...
_MESSAGE_SERIALIZERS = {"json": serialize_json, "blob": serialize_blob}
...
class AlertBroker:
        ...
        if "r" in mode:
            ccfg = {
                **cfg,
                ...
                "default.topic.config": {"auto.offset.reset": start_at},
    ...
    def _filtered_stream(self, mapper, filter, timeout):
        ...
        for msgs in itertools.chain(
            [self._buffer.values()], self._raw_stream(timeout=timeout)
        ):
            if not is_heartbeat(msgs):
                self._buffer = dict(enumerate(msgs))

                # process the messages on the workers
                for i, (rec, meta) in enumerate(
                    mapper(ParseAndFilter(parser=self._parser, filter=filter), msgs)
                ):
    ...
    def _commit_to_kafka(self, defer=True):
        ...
        if self._committed and (not defer or now - last > 5.0):
            print("COMMITTING", file=sys.stderr)
            tp = [
                TopicPartition(_topic, _part, _offs + 1)
    ...
    def _stream(self, filter, mapper, progress, timeout, limit):
        ...
        nread = 0
        for rec, meta in self._filtered_stream(
            mapper=mapper, filter=filter, timeout=timeout
        ):
    ...
    def __call__(
        self, filter=None, pool=None, progress=False, timeout=None, limit=None
    ):
        if pool:
        ...
        yield from self._stream(
            filter, mapper=mapper, progress=progress, timeout=timeout, limit=limit
        )
...
if __name__ == "__main__":
    ...
    try:
        ...
        with Pool(5) as workers:
            with AlertBroker(
                "kafka://broker0.do.alerts.wtf/test6", start_at="earliest"
            ) as stream:
                for nread, (idx, rec) in enumerate(
                    stream(filter=my_filter, pool=workers, progress=True, timeout=10),
                    start=1,
                ):
